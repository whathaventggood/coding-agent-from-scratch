...
为避免大文件或连续工具输出无限扩大模型消息历史，Agent 回传给模型的单次工具结果最多保留前 4000 个字符，并附加明确的截断标记。该限制只作用于模型消息，不会截断磁盘文件或修改工具的原始执行结果。当前版本尚不支持按行分块继续读取大文件；需要查看截断部分时，可先使用关键词搜索缩小范围。

`read_file` 支持可选的 `start_line` 和 `max_lines` 参数，用于按行分段读取大文件。行号从 1 开始，单次最多读取 200 行；返回结果会说明本次读取范围和文件总行数。未提供这两个参数时仍保持原来的整文件读取行为。分段结果如果仍超过 4000 个字符，回传模型时依然会触发单次工具结果截断。
## 运行 DeepSeek 读取示例
